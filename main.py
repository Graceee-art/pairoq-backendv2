...
"""
Pairoq API v2.1.1 — Production Hotfix
...
app = FastAPI(title="Pairoq API", version="2.1.1")
app.add_middleware(
    CORSMiddleware,
...
def safe_percentile(arr: np.ndarray, q: float, fallback: float = 1.0) -> float:
    ...
    if valid.size == 0:
        return fallback    return float(np.percentile(valid, q))
    ...
    for line in lines[4:]:
        ...
        if len(parts) < 3:
            ...
            if x == 0 and n == 0 and rho == 0:
                break            if rho > 0 and np.isfinite(rho):
                points.append((x, n, rho))
        except (ValueError, TypeError):
            continue    if not points:
    ...
    for ri, row_pts in enumerate(rows_data):
        for ci, (x, n, rho) in enumerate(row_pts):
            if ci < n_cols and np.isfinite(rho) and rho > 0.01:
                grid[ri, ci] = rho
                quality[ri, ci] = True        if row_pts and len(row_pts) < n_cols:
            last_val = row_pts[-1][2]
            for ci in range(len(row_pts), n_cols):
                grid[ri, ci] = last_val    depths = wenner_depths(n_levels, spacing)
...
def parse_csv_generic(content: bytes, spacing: float):
    ...
    for line in csv.reader(io.StringIO(text)):
        try:
            ...
            continue    if not rows:
        raise ValueError("No valid numeric data found.")
    n_levels = len(rows)
    max_cols = max(len(r) for r in rows)
    grid = np.array([r + [r[-1]] * (max_cols - len(r)) for r in rows], dtype=float)
    if (grid > 0).any():
        grid = sanitize(grid, fill=np.nanmedian(grid[grid > 0]))
    else:
        grid = sanitize(grid, fill=1.0)
...
def parse_syscal_csv(content: bytes, spacing: float):
    ...
    if rho_col is None:
        raise ValueError("Cannot find resistivity column.")
    points = []
    for i, row in enumerate(rows_raw):
        try:
            rho = float(row[rho_col])
            x = float(row[x_col]) if x_col else i * spacing
            n = int(float(row[n_col])) if n_col else 1            if np.isfinite(rho) and rho > 0:
                points.append((x, n, rho))
        except (ValueError, KeyError, TypeError):
            continue    if not points:
        raise ValueError("No valid resistivity data.")
    return _triplets_to_grid(points, spacing)
...
def parse_file(content: bytes, filename: str, spacing: float):
    ...
    if any(k in first_line for k in ['rho', 'resistivity', 'app_res', 'ra,']):
        try:
            ...
            pass    return parse_csv_generic(content, spacing)
...
def regularized_inversion(
    apparent: np.ndarray, depths: np.ndarray, quality: np.ndarray, spacing: float,
    iterations: int = 6, smoothness: float = 0.5, convergence_thr: float = 2.0
):
    ...
    for r in range(rows):
        for c in range(cols):
            if not quality[r, c]:
                ...
                model[r, c] = np.mean(neighbors) if neighbors else np.mean(apparent[quality]) if quality.any() else 1.0    model = sanitize(model, fill=1.0)
    sens = depth_sensitivity(depths)
    valid_data = apparent[quality] if quality.any() else apparent.flatten()
    valid_min = max(float(np.percentile(valid_data, 2)) * 0.3, 0.01)
    valid_max = float(np.percentile(valid_data, 98)) * 3.0
    rms_log = []
    prev_rms = None
    diverge_count = 0
    converged = False    for it in range(iterations):
        base_damp = smoothness / (1.0 + it * 0.4)
        if diverge_count > 0:
            base_damp *= (0.5 ** diverge_count)
        base_damp = float(np.clip(base_damp, 0.02, 0.8))
        for r in range(rows):
            sigma = 0.4 + (1.0 - sens[r]) * 1.2
            sm = lateral_smooth(model[r, :], sigma)
            model[r, :] = (1 - base_damp) * model[r, :] + base_damp * sm        for c in range(cols):
            col = model[:, c].copy()
            for r in range(1, rows - 1):
                w = float(np.clip(base_damp * (1.0 - sens[r]) * 0.35, 0, 0.5))
                vert = 0.25*col[r-1] + 0.5*col[r] + 0.25*col[r+1]
                model[r, c] = (1-w)*col[r] + w*vert        residual = sanitize(apparent - model)
        for r in range(rows):
            corr_w = float(sens[r]) * 0.12
            model[r, quality[r, :]] += residual[r, quality[r, :]] * corr_w        model = np.clip(model, valid_min, valid_max)
        ...
        if rms < convergence_thr:
            converged = True
            break    residuals_rel = np.abs(sanitize(model - apparent)) / (sanitize(apparent) + 1e-9)
...
def compute_confidence(rms: float, converged: bool, completeness: float, n_cols: int, n_levels: int) -> float:
    ...
    return round(float(np.clip(0.40*rms_score + 0.20*conv_score + 0.25*completeness + 0.15*coverage, 0, 1)), 4)

# ═══════════════════════════════════════════════════════════════
# INTERPRETATION
# ═══════════════════════════════════════════════════════════════
RESIS_TABLE = [
    (0,     10,    "Very Low",  "highly conductive anomaly",     "consistent with saline water, clay-rich sediments, or metallic mineralization"),
    (10,    30,    "Low",       "conductive zone",     "possible saturated sediments, clay layer, or weathered material"),
    (30,    100,   "Moderate",  "intermediate resistivity zone",     "may indicate partially saturated sediments or alluvial deposits"),
    (100,   300,   "Elevated",  "moderately resistive zone",     "consistent with dry sediments or consolidated material"),
    (300,   1000,  "High",      "resistive anomaly",     "possible consolidated rock, dry coarse sediment, or resistive intrusion"),
    (1000,  1e9,   "Very High", "highly resistive anomaly",     "consistent with crystalline rock or dry resistive bedrock"),
]
...
def interpret(model: np.ndarray, uncertainty: np.ndarray, depths: np.ndarray):
    ...
    for label_idx, (mn, mx, cls, desc, ctx) in enumerate(RESIS_TABLE):
        ...
        if not mask.any():
            continue        if cls in seen_classes:
            continue        seen_classes.add(cls)
        vals = flat[mask]
        ...
        zones.append({
            "label": display_label, "resistivity_class": cls, "geophysical_descriptor": desc, "geological_context": ctx,
            "range_ohm_m": [round(float(vals.min()),1), round(float(vals.max()),1)], "mean_ohm_m": round(mean_rho, 2),
            "uncertainty": round(mean_unc, 3), "confidence_qualifier": conf, "range": f"{vals.min():.0f}–{vals.max():.0f} Ω·m",
            "interpretation": ctx, "color": zone_color(cls),
        })
    log_m = safe_log10(model)
    valid_log = log_m[np.isfinite(log_m)]
    lmean = float(np.mean(valid_log)) if valid_log.size > 0 else 1.0
    lstd = float(np.std(valid_log)) if valid_log.size > 0 else 0.1
    anomalies = []
    for atype, amask in [
        ...
        anomalies.append({
            "type": atype, "geophysical_label": f"{atype.title()} anomaly ({desc})",
            "mean_resistivity_ohm_m": round(mean_rho, 1), "approximate_center_depth_m": round(mean_depth, 1),
            "geological_context": ctx, "confidence_qualifier": ("moderate-to-high" if mean_unc<0.25 else "moderate" if mean_unc<0.5 else "low"),
...
def render(model, uncertainty, depths, distances, rms_log, converged,
    ...
    if log_max <= log_min:
        log_max = log_min + 1.0    norm = mcolors.Normalize(vmin=log_min, vmax=log_max)
    
    # HOTFIX: Gunakan indexing='ij' agar orientasi koordinat konsisten dengan struktur penampang 2D
    X, Y = np.meshgrid(distances, depths, indexing='xy')
    ...
    try:
        ...
        # fallback koordinat eksternal khusus untuk imshow
        extent_val = [distances[0], distances[-1], depths[-1], depths[0]]

        if display_mode == 'contoured':
            levels_fill = np.linspace(log_min, log_max, 20)
            ax.contourf(X, Y, log_m, levels=levels_fill, cmap=CMAP, norm=norm, extend='both')
            cs = ax.contour(X, Y, log_m, levels=levels_fill[::3], colors='black', linewidths=0.9, alpha=0.7)
            try:
                ax.clabel(cs, cs.levels[::2], inline=True, fontsize=7, fmt=lambda x: f'{10**x:.0f}', colors='black', inline_spacing=2)
            except Exception:
                pass
        elif display_mode == 'hybrid':
            ax.imshow(log_m, aspect='auto', origin='upper', extent=extent_val, cmap=CMAP, norm=norm, interpolation='bilinear')
            cs = ax.contour(X, Y, log_m, levels=np.linspace(log_min, log_max, 12)[1:-1], colors='black', linewidths=0.8, alpha=0.75)
            try:
                ax.clabel(cs, cs.levels[::2], inline=True, fontsize=7, fmt=lambda x: f'{10**x:.0f}', colors='black', inline_spacing=2)
            except Exception:
                pass
            ...
            ax.imshow(log_m, aspect='auto', origin='upper', extent=extent_val, cmap=CMAP, norm=norm, interpolation='bilinear')
            try:
                ax.contour(X, Y, log_m, levels=np.linspace(log_min, log_max, 10)[1:-1], colors='black', linewidths=0.6, alpha=0.5)
            except Exception:
                pass

        if (uncertainty > 0.70).any():
            try:
                ax.contourf(X, Y, uncertainty, levels=[0.70, 1.01], colors='white', alpha=0.25, hatches=['////'])
            except Exception:
                pass
        ...
        fig.text(0.5, 0.95, f'Model Resistivity Section  —  {filename}', ha='center', fontsize=12, fontweight='bold')
        fig.text(0.5, 0.91, f'RMS = {rms_val:.2f}%  |  Iter: {len(rms_log)}  |  {conv_str}', ha='center', fontsize=9, color='#444')
        conf_pct = int(confidence * 100)
        conf_color = ('#16a34a' if conf_pct >= 70 else '#d97706' if conf_pct >= 50 else '#dc2626')
        fig.text(0.06, 0.91, f'Confidence: {conf_pct}%', fontsize=8, color=conf_color, fontweight='bold')
...
def root():
    return {
        "service": "Pairoq Geophysical API", "version": "2.1.1",
        "inversion": "regularized_least_squares_approximation",
        "disclaimer": "Preliminary interpretation tool. Requires professional validation.",
    }
...
@app.post("/process")
async def process(
    file: UploadFile = File(...), spacing: float = Form(5.0), iterations: int = Form(6),
    array_type: str = Form("wenner_alpha"), display_mode: str = Form("hybrid"),
):
    t0 = time.time()
    ...
    if display_mode not in ("smooth", "contoured", "hybrid"):
        display_mode = "hybrid"    content = await file.read()
    ...
    if grid.shape[0] < 2 or grid.shape[1] < 3:
        raise HTTPException(400, "Dataset too small. Need at least 2 depth levels and 3 electrodes.")
    ...
    return JSONResponse({
        ...
        "stats": {
            "points_total": int(quality.size), "points_valid": int(quality.sum()), "data_completeness": round(completeness, 4),
            "n_levels": int(grid.shape[0]), "n_electrodes_estimated": int(grid.shape[1] + 3*grid.shape[0]),
            "min_resistivity_ohm_m": round(float(safe_percentile(valid_data, 0)), 2), "max_resistivity_ohm_m": round(float(safe_percentile(valid_data, 100)), 2),
            "mean_resistivity_ohm_m": round(float(np.mean(valid_data)), 2), "max_depth_m": round(float(depths[-1]), 2),
            "electrode_spacing_m": spacing, "array_type": array_type,
        },
        "inversion": {
            "method": "regularized_least_squares_approximation", "iterations_run": len(rms_log), "converged": converged,
            "rms_final_pct": rms_log[-1] if rms_log else None, "rms_per_iteration": rms_log, "confidence_score": confidence,
        },
        "zones": zones, "anomalies": anomalies,
        "depths_m": [round(float(d), 2) for d in depths], "distances_m": [round(float(d), 2) for d in distances],
        "rms_final": rms_log[-1] if rms_log else None, "rms_history": rms_log,
    })
PYEOF
echo "Hotfix done, running stability validation..."
